...
public class CalculatorTest {
    ...
    public void maxRoiCalculator() {
        ...
        Map<String, City> cites = new HashMap<>(cities.size());
        ...
        // 大理
        List<Route> dali = calculator.calculate("大理");
        BigDecimal total = BigDecimal.ZERO;
        for (Route dl : dali) {
            total = total.add(dl.getRoi());
        }
        System.out.printf("大理：totalRoi : %s - routes : %s%n", total, gson.toJson(dali));
        // 苏州
        List<Route> suzhou = calculator.calculate("苏州");
        total = BigDecimal.ZERO;
        for (Route sz : suzhou) {
            total = total.add(sz.getRoi());
        }
        System.out.printf("苏州：totalRoi : %s - routes : %s%n", total, gson.toJson(suzhou));
        // 洛阳
        List<Route> luoyang = calculator.calculate("洛阳");
        total = BigDecimal.ZERO;
        for (Route ly : luoyang) {
            total = total.add(ly.getRoi());
        }
        System.out.printf("洛阳：totalRoi : %s - routes : %s%n", total, gson.toJson(luoyang));
    }
}
